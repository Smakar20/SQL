...
The empty cell data for columns with less than the maximum number of names per occupation (in this case, the Professor and Actor columns) 
are filled with NULL values.
*/

SELECT Doctor,Professor, Singer, Actor FROM 
( 
SELECT name, occupation, DENSE_RANK() OVER (PARTITION BY occupation ORDER BY name) as rnk FROM occupations ) 
PIVOT 
( 
MAX(name) 
