...
The results of the second query are ascendingly ordered first by number of names corresponding to each profession, and then alphabetically 
by profession.
*/

select name || '(' || upper(substr(occupation,0,1)) || ')' from occupations order by name; 
select 'There are a total of ' || count(occupation) || ' ' || lower(occupation) || decode(count(occupation),1,'.','s.') from 
occupations group by occupation order by count(occupation) asc, occupation asc;
